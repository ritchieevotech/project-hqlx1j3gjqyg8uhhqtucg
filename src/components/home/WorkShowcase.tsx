...
export const WorkShowcase = () => {
  return (
    <section className="py-24 bg-white overflow-hidden">
      ...
      <div className="relative -rotate-3 scale-110">
        <div className="flex space-x-8 overflow-hidden">
          <motion.div 
            className="flex space-x-8 whitespace-nowrap"
            animate={{ x: [0, -1344] }} // (400px width + 32px gap) * 3 images
            transition={{ 
              duration: 20, 
              repeat: Infinity, 
              ease: "linear" 
          ...
          >
            {[...showcaseImages, ...showcaseImages, ...showcaseImages, ...showcaseImages].map((src, i) => (
              <div 
                key={`${src}-${i}`} 
